...
class Location:
    ...
    def loc(self, address):
        print(f"First address is {address}")

        # Build the API URL
        url = config['Link']['geoapify_url_first'] + address2.replace(" ", "%20") + config['Link']['geoapify_url_last']
        # url2 = f"https://api.geoapify.com/v1/geocode/search?text={address2}&limit=1&apiKey={API_KEY}"
        # print(url)
        response2 = requests.get(url)
